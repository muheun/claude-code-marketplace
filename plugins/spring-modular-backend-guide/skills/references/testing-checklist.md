...
- Paged list controller tests follow `web-and-app.md` (Search Binding): they assert `page`, `pageSize`, and filters bind as query parameters onto `*Search`, not a `new *Search(page, pageSize, ...)` from scalar `@RequestParam` values. Extra controller-set values are asserted as HTTP or principal input assigned onto Search after bind. Do not retest service `calcPaging` through the controller.
