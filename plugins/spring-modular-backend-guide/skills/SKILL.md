...
- Naming, enum parsing, response envelopes, and adapter registration are design/review rules first. Import ordering and other formatting are write-time rules for code you produce; review findings report design and behavior problems, and formatting stays with the project formatter (see Review Scope in `references/anti-patterns.md`). Add static enforcement only when the project treats the policy as stable or the task explicitly touches that boundary.
